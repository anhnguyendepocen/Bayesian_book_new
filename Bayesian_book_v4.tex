...
model {
	...
	\end{minted}
One important thing to notice about the above is the use of so-called sampling statements written by ``~''. The term ``sampling statement'' is suggestive and not to be taken literally \cite{carpenter2016stan}. In the above we use a sampling statement for the individual elements of $Y$ because we assumed that the likelihood was normal: $Y\sim N(\mu,\sigma)$. Similarly, we assumed that the priors on the parameters were normal and gamma respectively, as suggested by the above code.

However do not think that Stan actually does the independent sampling that these statements seem to suggest. 

In other blocks to access an individual element in this data array we just use square parentheses, so Y[1]

...
Furthermore, it turns out the denominator is also a valid probability density\footnote{Due to the fact that we have removed the $\theta$ dependence that confounds attempts to view the numerator as one.}, meaning that we can calculate the counterfactual $P(negative) = 1 - P(positive) = \frac{29}{40}$. We need to be careful with interpreting this last result, since it didn't actually occur. It's best to think of $P(negative)$ as the probability that we would assign to an individual testing negative before we carry out the test.
...
The denominator does not really have a use in itself, without comparing it to something else.

It is actually a \textit{marginal} probability density that is obtained by summing/integrating out the dependence on the parameter(s) of the joint density $P(data,\theta)$.

Include the 3D visualisation of $P(data,\theta)$ in discrete and continuous cases.
The Bayes' factor/probability of data and its use in model comparison. P64. Why doesn't Kruscke use the Bayes' factor when comparing models? See 184 in Gelman. 

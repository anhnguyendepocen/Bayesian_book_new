...
We frequently receive partial information by observing only part of the system in which we are interested. In our coin example, we might flip of one of the coins finding it coming up heads, and on this basis then update our probabilities of heads or tails for the other. Alternatively, in the foot size - IQ example described before, we might measure an individual's shoe size, and determine the probabilities across a continuum of IQ values. 

In probability, when we observe one variable, and reformulate the probability distribution for the other variable, we say that we are deriving the \textit{conditional} distribution of the latter. \textit{Conditional} refers to the fact that we are deriving the a probability distribution of one variable, \textit{conditional} on the value of the other(s).

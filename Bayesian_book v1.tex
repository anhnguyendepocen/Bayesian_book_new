...
What is the benefit of a particular drug to treatment success? What is the mean weight of a US female? How does income inequality vary in post-communist countries? We frequently use a sample of data to draw conclusions about the wider world. The issue with generalising from a sample to a population is that our data is subject to variance due to the way in which we select our sample, as well as any variance that is due to the actual process of interest. If we find in a sample of 100 individuals that the mean score of a particular test is 112, how does this generalise to the population of \textit{all} individuals taking this test? Is it that the true population mean is 112, or did we just happen to pick a sample of people who show higher than average performance on this test?

Statistical inference provides us with a logical framework which allows us to extrapolate from the small to the big, dealing with this issue of \textit{sampling variation}. This contrasts with descriptive statistics, which simply aims to describe a dataset, without reference to the fact that it comes from a larger population.
...
\section{The world according to frequentist inference}
In frequentist inference, we suppose that our sample of data is the result of one of an infinite number of exactly-repeated experiments. Any conclusions that we draw from this approach are hence based on the supposition that events occur with probabilities, which represent long-run frequencies. For example, if we flip a coin, we might assume that any sequence of outcomes we obtain is indicative of the results that we might obtain if we were to conduct the experiment an infinite number of times. Further, we might take the proportion of heads observed in this infinite set of throws as defining the probability of obtaining a 'heads'. We suppose that this probability actually exists, and is fixed for each set of coin-throws that we carry out.

In general, in frequentist statistics assume that the data is the result of \textit{sampling variation}, from a fixed, \textit{defined}, population distribution.  

\section{The world according to Bayesian inference}
The frequentist paradigm is not well-suited to answer what is meant by the statement, 'The probability of the Republicans winning the next Presidential election is 0.31'. 

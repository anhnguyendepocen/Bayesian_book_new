...
In order to do this, we essentially need to \textit{average} out the dependence of the other variable. In our coins example, if we are only interested in coin A, we can sum across the column values for coin B, obtaining the \textit{marginal} distribution of coin A, shown at the bottom of table \ref{tab:Intro_coinsMarginal}.

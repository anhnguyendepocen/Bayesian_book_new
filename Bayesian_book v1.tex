...
\begin{table}[htbp]
  \centering
    ...
    Submarine & \color{red}{$\left[$73\%}  & \color{red}{50\%}  & \color{red}{50\%$]$}  & 13\%  & \textbf{93}\% \\
    Boat  & \color{red}{$[$9\%   & \color{red}{25\%}  & \color{red}{25\%$]$}  & 16\%  & \textbf{79}\% \\
    Tank  & 0\%   & 25\%  & 25\%  & \color{red}{$\left[$\color{red}{66\%$]$}  & \textbf{78}\% \\
...
From the prior distribution, $p(i)$, we would like to estimate the probability of particular rates of investment return. Mathematically, what we want is the marginal distribution of $x$ - $p(x)$. Fortunately, we know from section \ref{sec:Intro_marginal}, that we can get a marginal distribution from a joint distribution by summing (for a discrete parameter):

\begin{equation}\label{eq:Posterior_priorPredictiveDistribution}
\begin{align}
...
This distribution essentially weights each of the 
...
Decide on graphs showing the construction of the prior predictive distribution.

\subsection{Example: both continuous}

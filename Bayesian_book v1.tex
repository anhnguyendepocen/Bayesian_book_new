...
\caption{.}\label{fig:Intro_varianceLottery}
\end{figure} 
...
Suppose we are stationed in a radio control tower, on a cliff of a rocky isle, and we are tasked with guiding the coast guard towards a distressed vessel, which we have been told radioed in earlier from an unknown location. We know from our prior experience that ships tend to travel West towards the island from a nearby port, and sometimes run into trouble in the most treacherous seas that start about 50km from the coastline. This stretch of sea is ...

Suppose we are stationed in a radio control tower, on a cliff of a rocky isle. We are tasked with guiding the coast guard towards a sinking vessel, which signalled half an hour ago from an Easterly direction, 30km away, travelling West towards the island. We know that the ship's maximum speed is 20kmph, meaning that the closest the ship can be to the island is 10km away. We also know that it will not stray from its heading towards the island, since it is stuck on autopilot. What we don't know is whether the ship has engine trouble, meaning it could be travelling slower than its maximum speed.

In Bayesian statistics the posterior distribution summarises the combination of our pre-study, and post-analysis knowledge about a given situation, and is used as the starting point for any further analysis or descriptions of our results. This central role of the posterior is why we devote so much energy to its derivation (and why I have devoted an entire part of this book to understanding how we can use Bayes' rule to find it). I introduce now a toy example, to help understand the significance of a prior, and posterior density.
...
Maybe do as a video. Stack exchange 'whats the difference between a confidence interval and a credible interval cookie jar example. Illustrate the different importance of each, dependent on your goals.

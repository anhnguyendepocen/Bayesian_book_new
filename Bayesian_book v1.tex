...
Fortunately, previous code-breakers have managed to decode a significant proportion of messages, and have tallied up the proportions of communications from each source, which resulted in a particular attack destination (see figure \ref{tab:Posterior_confidenceIntervalHistoric}).
...
\begin{table}[htbp]
  \centering
    ...
    Submarine & \color{red}{$[$80\%}  & \color{red}{50\%}  & \color{red}{50\%$]$}  & 13\%  & \textbf{93\%} \\
    Boat  & 9\%   & \color{red}{$[$15\%}  & \color{red}{25\%}  & \color{red}{16\%$]$}  & \textbf{86\%} \\
    Tank  & 0\%   & 25\%  & 25\%  & \color{red}{$[$66\%$]$}  & \textbf{57\%} \\
    Aircraft & 11\%   & \color{red}{$[$10\%$]$}  & 0\%   & 5\%   & \textbf{38\%} \\
    \bottomrule
    \textbf{Coverage} & \textbf{80\%}  & \textbf{75\%}  & \textbf{75\%}  & \textbf{82\%} &  \\
    \end{tabular}%
...
\begin{table}[htbp]
  \centering
    ...
    Submarine & \color{red}{$\left[$80\%}  & \color{red}{50\%}  & \color{red}{50\%$]$}  & 13\%  & \textbf{93}\% \\
    Boat  & \color{red}{$[$9\%   & \color{red}{15\%}  & \color{red}{25\%$]$}  & 16\%  & \textbf{75}\% \\
    Tank  & 0\%   & 25\%  & \color{red}{$[$25\%}  & \color{red}{66\%$]$}  & \textbf{78}\% \\
    Aircraft & \color{red}{$[$11\%}   & \color{red}{10\%$]$}  & 0\%   & 5\%   &\textbf{ 81}\% \\
    \bottomrule
...
The difference between these two measures is subtle. In fact, as is often the case, the intervals are actually similar, and overlap considerably. But which should we choose? Using the classical confidence intervals, we are assured that whatever mode of attack the enemy chooses, our interval will contain the true attack mode at least 75\% of the time. A Bayesian would criticise the confidence interval for the case of the \textit{Aircraft} communication method, since it only gets the correct destination correct 38\% of the time. This could potentially be costly if attacking communicated by this method are particularly devastating. A Frequentist would argue that for attacks on airfields, the Bayesian confidence intervals only correct this as the attack destination 66\% of the time. Again, if these types of attack are particularly costly, then this interval might not be ideal\footnote{If we were to assign costs to each of these attack events, then we could work out optimal intervals, although this is the realm of Bayesian Decision Theory; not covered in this text.}.

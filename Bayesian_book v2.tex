...
\subsection{Multivariate normal and multivariate t}
...
\item Requires specification of a vector of means, and a matrix of parameters (either directly or by setting priors), which describes the covariance between elements of the vector.
...
Imagine you get a job at a prestigious hedge fund as an analyst. Your manager gives you your first job: to model the risk of a portfolio containing the derivatives (complex financial instruments that allow for disproportionate profit \textit{and} loss) on stocks of 2 different companies within a particular industry sector. You are told that if both stocks have a daily loss of 10\% or more, that the portfolio goes bust. Further, your boss tells you that a risk of $\frac{1}{1000}$ is acceptable, but no more. You are given the historical daily returns of the stocks shown in the left hand panel of figure , and told to get modelling!

Excited about your first pay check, you get to work. Looking at the historical returns you notice that when one series tends to go up, the other does as well. So the model that you specify will need to take into account this covariance structure. Bearing this in mind, you go away and do some research, and decide that a reasonable model for the returns of the two stocks, $\boldsymbol{r}$, is a \textit{multivariate normal}:
...
\subsubsection{Multivariate normal, multivariate t, and mixtures of multivariate normals}
The following is somewhat more (at least mathematically) advanced than the preceding distributions. Unless you are in need of multivariate models, and have a grasp of vectors, and matrices, then it can be left until you start Part \ref{part:regressionHierarchical}, after which we will start to use these concepts.
...
\item By their nature the parameters of these matrices are constrained.
\item Mostly useful in hierarchical models, although can be used in non-hierarchical settings.
\item A better alternative to the Inverse-Wishart, particularly when conjugacy is not required.
\end{enumerate}

Read paper on visualising covariance matrices and BDA p83 onwards.


Better alternative to Wishart. Note that in the manual on page 61 - it should say uniform, not \textit{identity}. Also note that it should never explicitly be used in practice, and factors used instead. \url{https://groups.google.com/forum/?fromgroups#!searchin/stan-users/lkj/stan-users/F23kBsgqGcY/Yu3eOMBEKgYJ}

\subsubsection{Wishart and inverse-Wishart}
On p60 of the manual it is said that this distribution is conjugate to the multivariate normal - see Gelman and Hill.

...
\section{Chapter mission statement}
At the end of this chapter the reader will understand the powerful methods that a Bayesian has at their disposal, which allow detailed evaluation of the performance of a given model, as well as comparison between models. We will also discuss how to go about testing hypotheses about your model in the Bayesian framework.
...
The Bayesian testing framework which allows us to test a model is known as \textit{posterior predictive checks} (PPCs). The idea behind this umbrella of methods is simple. You should be able to use your fitted model to generate fake data that looks indistinguishable from the real thing. In practice, this is not usually fully-attainable, so it is desirable to choose a model that sufficiently captures the variation in the data in which you are most interested. Choice of the particular features to use for evaluation and comparison hence becomes the most important part of Bayesian model checking. The beauty of PPCs is that they allow us to understand in what ways a model is good, and in what ways it is deficient. Examining models through this multidimensional lens is both more rewarding than widely used evaluation criteria, and easier to understand.

However, there will inevitably be times when we are pushed, either due to policy-makers or an innate desire for simplicity, to choose between two models which perform similarly on PPCs. Recently, there has been a movement to introduce uni-dimensional model evaluation criteria that more closely follow the Bayesian spirit than traditional AICs or DICs. Towards the end of this chapter we shall introduce these relatively new additions to the modeller's arsenal, and discuss their use.

We will also, in passing, discuss the ways in which Bayesians can go about testing hypotheses about a given model, or data, and we shall see that this is much more fluid, and less arcane than the classical equivalents. We'll also see that there is no need for the binary null hypothesis test that we all learned at school, since Bayesian posteriors adequately capture all we need to know about the question of interest.
...
\section{Univariate generic model evaluation criteria: AIC, Deviance, WAIC and LOO}
...
\section{Hypothesis testing in the Bayesian world}

...
\begin{equation}
\begin{align}
\mathrm{E(\lambda)} &= \int\limits_{0}^{\infty} \lambda p(\lambda|\boldsymbol{x}) \mathrm{d}\lambda\\
...
So if we had a way of making our continuous parameter problems discrete, we could then easily solve for the posterior. Typically, this discretisation process is an approximation, where we suppose that our prior and likelihood only exist at a finite set of points on a grid, resulting in an approximate posterior that is also discrete. 

As an example, suppose that we continue our BSE example, where we had a Poisson likelihood, and a log-normal prior.  

\section{Integrating using quadrature}

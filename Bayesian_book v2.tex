...
\subsection{Recap - posterior predictive distributions}
...
This measure is easy to calculate, but does not have any theoretical justification (apart from normal likelihood models), which limits its scope. A more theoretically motivated measure would be to consider the likelihood $p(\boldsymbol{y}|\theta)$, but more commonly we use its close cousin, the log-likelihood, $log p(\boldsymbol{y}|\theta)$. Since the log function is a monotonic transformation the score obtained from using the log-likelihood will mirror the likelihood. However, it is favoured to use the log-likelihood because of its connection with a concept called the Kullback-Leiber (KL) divergence measure, which can be used to find the difference between the true density and estimated. If we maximise the log-likelihood, it is approximately equivalent to finding the density has the lowest KL divergence from the true density (the approximation becomes better the larger the sample size). The log-likelihood in this case is also referred to as the \textit{log predictive density}. 

Due to the generality of the log-likelihood, it is favoured as a measure of a model's predictive accuracy. However, seeing as we are Bayesians, you might ask why don't we use the log posterior rather than the likelihood? The reason for this is that we are interested in summarising the \textit{fit} of the model to data, and the prior is only useful in that it helps us to estimate better the parameters $\theta$ of a distribution \cite{gelman2014understanding}. Essentially all information in the prior has already been passed through to the posterior of $\theta$, and if we were to use it again in evaluating the model's predictive accuracy, we would be using it twice unnecessarily. 


\subsection{The ideal measure of a model's predictive accuracy}\label{sec:Evaluation_idealAccuracy}
In practice we don't know the true parameter value $\theta$ (nor are we sure such a value exists), so we cannot simply calculate the log predictive density $log\left[ p(\boldsymbol{y}|\theta)\right]$. As such, we need some way of incorporating our uncertainty in its value into our measure of predictive accuracy. A sensible thing to do would be to estimate its expectation (see section \ref{sec:Probability_meanVariance}) over all the values of the posterior:

\begin{equation}\label{eq:Evaluation_logPosterior}
log\left[\mathrm{E_{posterior}}(p(\boldsymbol{y^{new}}))\right] = log \left[\int\limits_{\theta\in\Theta} p(\boldsymbol{y^{new}}|\theta)p(\theta|\boldsymbol{y})\mathrm{d}\theta\right]
\end{equation}

where $\mathrm{E_{posterior}}$ denotes expectation under the posterior distribution of $\theta$. simplicity imagine we start by considering a single new data point $y^{new}$, which amounts to replacing $\boldsymbol{y^{new}}$ by $y^{new}$ above. Usually we don't have to hand the extra data, and hence the new data $y^{new}_1$ are unknown. If we knew the true distribution $f(y)$ for a single data point, then we could hence evaluate the expectation of (\ref{eq:Evaluation_logPosterior}), which Gelman et al. (2013) call the expected log predictive density (elpd):

\begin{equation}
\begin{align}
elpd &= \mathrm{E_f}\left(log\left[\mathrm{E_{posterior}}(p(\boldsymbol{y^{new}}|\theta))\right]\right)\\
&= \int\limits_{y^{new}\in Y} log\left[\mathrm{E_{posterior}}(p(\boldsymbol{y^{new}}|\theta))\right] f(y^{new})\mathrm{d}y^{new}
\end{align}
\end{equation}

where $\mathrm{E_f}$ denotes expectation under the true data distribution $f(y)$. If we then consider the elpd for our $n$ new data points, taken one at a time, we have what Gelman  et al. (2013) calls the expected log pointwise predictive density (elppd):
...
Using the pointwise measure defined above is preferable to using the full joint predictive distribution $\mathrm{E_f}\left(p(\boldsymbol{y^{new}}|\theta)\right)$ is that it can enable a range of generalisable expressions which represent our out of sample error \cite{gelman2013bayesian}.
...
Ultimately, we would like a measure of predictive accuracy that best approximates the elppd defined in (\ref{eq:Evaluation_elppd}), given that we do not have new data, $\boldsymbol{y^{new}}$ nor the true density of a datum $f(y)$. To tackle this we firstly substitute the actual data $\boldsymbol{y}$ in for the future data $\boldsymbol{y^{new}}$, allowing us to calculate the log pointwise predictive density (lppd) of the observed data \cite{gelman2013bayesian}:

\begin{equation}
lppd = \sum\limits_{i=1}^{n} log\left(\int\limits_{\theta\in\Theta} p(y_i|\theta)p(\theta|\boldsymbol{y})\mathrm{d}\theta\right)
\end{equation}

where $p(\theta|\boldsymbol{y})$ is the posterior density. 
...
\section{Evaluating a models' accuracy}
